import bengaliCourses from "@/data/bengalicourse.json";
import hindiCourses from "@/data/hindicourses.json";
import odiaCourses from "@/data/odiacourses.json";

const allCourses = [...bengaliCourses, ...hindiCourses, ...odiaCourses];
const courses = [];
const seenIds = new Set();
for (const course of allCourses) {
  const baseId = course.id.replace(/^hindi-/, "").replace(/^odia-/, "");
  if (!seenIds.has(baseId)) {
    // We store the course with its baseId to ensure everything works smoothly
    courses.push({ ...course, id: baseId });
    seenIds.add(baseId);
  }
}
...
export function getCourseContentByLanguage(id) {
  const baseId = id.replace(/^hindi-/, "");
  const hindiId = `hindi-${baseId}`;

  const mapCourse = (course) => (course ? normalizeCourseResources(course.resources) : null);

  return {
    bengali: mapCourse(bengaliCourses.find((c) => c.id === baseId)),
    hindi: mapCourse(hindiCourses.find((c) => c.id === hindiId)),
    odia: mapCourse(odiaCourses.find((c) => c.id === baseId)),
  };
}
