...
// Plain‑text comparison with constant‑time safety.
export function comparePlainPassword(password, stored) {
  return timingSafeEqualStrings(String(password ?? ""), String(stored ?? ""));
}
...
export async function verifyMarketerPassword(password, passwordHash) {
  ...
  if (parts.length !== 4) {
    // Not a PBKDF2 hash, treat as plain text stored password
    return timingSafeEqualStrings(String(password ?? ""), raw);
  }
