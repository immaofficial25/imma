...
def upsert_from_salesforce_case(connector_id: str, payload: IncidentPayload) -> str:
    """Take a Salesforce Case payload and upsert a local incident."""
    external_id = payload.external_id
    external_key = payload.external_key

    # Look up existing sync link
    link = _find_sync_state(connector_id, "case", external_id)
    ...
    if link and link.get("internal_id"):
        ...
        logger.info(f"[salesforce-sync] updated incident {incident_id} from {external_key}")
        return incident_id

    # ---- Create a new local incident ----------------------------------------
    local_payload = {
        "subject": payload.subject[:200] or f"Salesforce case {external_key}",
        "description": payload.description or "(no description)",
        "caller": payload.caller or "Salesforce sync",
    ...
    logger.info(f"[salesforce-sync] created incident {incident_id} from {external_key}")
    return incident_id
