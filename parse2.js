...
classes.forEach((classData, idx) => {
  const courseIdx = idx; // 0 is Class 4, etc
  if(courses[courseIdx]) {
     const regex = /(\d+)(?:st|nd|rd|th)\s+[Cc]lass\s+([A-Za-z]+)\s*\(([^)]+)\)\s*(meet\.google\.com\/[a-z-]+)/g;
     const liveClasses = [];
     let match;
     while ((match = regex.exec(classData)) !== null) {
       liveClasses.push({
         class_no: parseInt(match[1], 10),
         day: match[2],
         time: match[3],
         meet_link: "https://" + match[4]
       });
     }
